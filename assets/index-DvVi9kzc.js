...
请提供准确、流畅的翻译。`}],m=JSON.parse(localStorage.getItem("customTemplates")||"[]");function z(e,o,s){const r={id:`custom-${Date.now()}`,name:e,category:o,icon:"⭐",systemPrompt:s,isCustom:!0};return m.push(r),localStorage.setItem("customTemplates",JSON.stringify(m)),r}function A(e){const o=m.findIndex(s=>s.id===e);o>-1&&(m.splice(o,1),localStorage.setItem("customTemplates",JSON.stringify(m)))}function u(){return[...P,...m]}function N(e,o,s,r,a){const i={id:Date.now(),timestamp:new Date().toISOString(),input:e,output:o,mode:s,model:r,templateName:a||"无模板",note:""};t.favorites.unshift(i),localStorage.setItem("favorites",JSON.stringify(t.favorites))}function j(e){t.favorites=t.favorites.filter(o=>o.id!==e),localStorage.setItem("favorites",JSON.stringify(t.favorites))}function C(e,o){const s=t.favorites.find(r=>r.id===e);s&&(s.note=o,localStorage.setItem("favorites",JSON.stringify(t.favorites)))}function L(){t.favorites=[],localStorage.setItem("favorites",JSON.stringify([]))}function n(e,o="success"){const s=document.createElement("div"),r={success:"from-green-400 to-emerald-500",error:"from-red-400 to-rose-500",info:"from-blue-400 to-cyan-500"};s.className=`fixed top-20 right-8 bg-gradient-to-r ${r[o]} text-white px-6 py-4 border-3 border-black font-bold depth-4 rounded-sm animate-slide-in-right z-50`,s.innerHTML=e,document.body.appendChild(s),setTimeout(()=>{s.style.animation="slide-out-right 0.3s ease-out",setTimeout(()=>s.remove(),300)},2e3)}function f(e,o){return e==="style"?t.favorites.some(s=>s.type==="style"&&s.name===o):e==="template"?t.favorites.some(s=>s.type==="template"&&(s.templateId===o||s.templateId===parseInt(o))):!1}function R(){return`
...
- 应用的框架：[RTD/APE/思维链等]
- 添加的要素：[角色/示例/约束等]
- 预期提升：[准确性/完整性/可控性]`;const i=t.apiUrl.startsWith("http://")?`https://cors-anywhere.herokuapp.com/${t.apiUrl}/chat/completions`:`${t.apiUrl}/chat/completions`,l=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t.apiKey}`},body:JSON.stringify({model:o,messages:[{role:"system",content:r},{role:"user",content:`请优化以下提示词需求：
