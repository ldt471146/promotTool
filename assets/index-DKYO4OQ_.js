...
请提供准确、流畅的翻译。`}],m=JSON.parse(localStorage.getItem("customTemplates")||"[]");function z(e,o,s){const r={id:`custom-${Date.now()}`,name:e,category:o,icon:"⭐",systemPrompt:s,isCustom:!0};return m.push(r),localStorage.setItem("customTemplates",JSON.stringify(m)),r}function A(e){const o=m.findIndex(s=>s.id===e);o>-1&&(m.splice(o,1),localStorage.setItem("customTemplates",JSON.stringify(m)))}function u(){return[...P,...m]}function N(e,o,s,r,a){const i={id:Date.now(),timestamp:new Date().toISOString(),input:e,output:o,mode:s,model:r,templateName:a||"无模板",note:""};t.favorites.unshift(i),localStorage.setItem("favorites",JSON.stringify(t.favorites))}function C(e){t.favorites=t.favorites.filter(o=>o.id!==e),localStorage.setItem("favorites",JSON.stringify(t.favorites))}function j(e,o){const s=t.favorites.find(r=>r.id===e);s&&(s.note=o,localStorage.setItem("favorites",JSON.stringify(t.favorites)))}function L(){t.favorites=[],localStorage.setItem("favorites",JSON.stringify([]))}function n(e,o="success"){const s=document.createElement("div"),r={success:"from-green-400 to-emerald-500",error:"from-red-400 to-rose-500",info:"from-blue-400 to-cyan-500"};s.className=`fixed top-20 right-8 bg-gradient-to-r ${r[o]} text-white px-6 py-4 border-3 border-black font-bold depth-4 rounded-sm animate-slide-in-right z-50`,s.innerHTML=e,document.body.appendChild(s),setTimeout(()=>{s.style.animation="slide-out-right 0.3s ease-out",setTimeout(()=>s.remove(),300)},2e3)}function f(e,o){return e==="style"?t.favorites.some(s=>s.type==="style"&&s.name===o):e==="template"?t.favorites.some(s=>s.type==="template"&&(s.templateId===o||s.templateId===parseInt(o))):!1}function R(){return`
  ...
  `}function h(){switch(t.currentView){case"optimizer":return S();case"styles":return F();case"templates":return K();case"favorites":return J();case"history":return _();case"settings":return D();default:return S()}}function F(){const{currentPage:e,pageSize:o,styleSearchKeyword:s}=t,r=s?w.filter(c=>c.name.includes(s)||c.nameEn.toLowerCase().includes(s.toLowerCase())||c.desc.includes(s)||c.keywords.some(p=>p.includes(s))):w,a=Math.ceil(r.length/o),i=(e-1)*o,l=i+o,b=r.slice(i,l);return`
    <div class="max-w-7xl mx-auto px-6 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ${b.map(c=>`
            <div class="border-2 border-black overflow-hidden hover:shadow-[4px_4px_0px_0px_rgba(255,0,110,1)] transition-all">
              ...
              <div class="p-4">
                <h3 class="font-bold text-lg mb-2">${c.name} / ${c.nameEn}</h3>
                <p class="text-sm text-gray-600 mb-3">${c.desc}</p>
                <div class="flex gap-2 mb-4">
                  ${c.keywords.slice(0,3).map(p=>`<span class="text-xs px-2 py-1 bg-gray-100">${p}</span>`).join("")}
                </div>
                <div class="flex gap-2">
                  <a href="${c.docUrl}" target="_blank" class="flex-1 border-2 border-black bg-black text-white font-bold py-2 text-sm text-center hover:bg-gray-800 transition-colors">查看文档</a>
                  <button onclick="favoriteStyle('${c.name}')" class="border-2 border-black ${f("style",c.name)?"bg-gradient-to-r from-yellow-300 via-amber-400 to-yellow-500 shadow-yellow animate-pulse-slow scale-110":"bg-gray-200 text-gray-500"} px-3 py-2 text-xl font-bold hover:scale-105 transition-all">${f("style",c.name)?"⭐":"☆"}</button>
                </div>
              </div>
            </div>
          `).join("")}
        ...
        <div class="flex justify-center items-center gap-2 mb-8">
          ...
          ${Array.from({length:a},(c,p)=>p+1).map(c=>`
            <button onclick="changePage(${c})" class="px-4 py-2 border-2 border-black font-bold ${c===e?"bg-black text-white":"hover:bg-gray-100"} transition-colors">${c}</button>
          `).join("")}
  ...
  `}function J(){const e=t.favorites.filter(r=>!r.type||r.type==="prompt"),o=t.favorites.filter(r=>r.type==="style"),s=t.favorites.filter(r=>r.type==="template");return`
...
- 应用的框架：[RTD/APE/思维链等]
- 添加的要素：[角色/示例/约束等]
- 预期提升：[准确性/完整性/可控性]`;const i=t.apiUrl.startsWith("http://")?`https://api.allorigins.win/raw?url=${encodeURIComponent(t.apiUrl+"/chat/completions")}`:`${t.apiUrl}/chat/completions`,l=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t.apiKey}`},body:JSON.stringify({model:o,messages:[{role:"system",content:r},{role:"user",content:`请优化以下提示词需求：

${e}`}],temperature:.7,max_tokens:2e3})});if(!l.ok)throw new Error(`API 错误: ${l.status}`);const b=await l.json();t.result=b.choices[0].message.content,O(e,t.result,t.optimizeMode,o)}catch(r){t.result=`❌ 优化失败：${r.message}

请检查设置中的 API 配置是否正确。`}t.isOptimizing=!1,d()};window.copyResult=()=>{navigator.clipboard.writeText(t.result),n("✅ 已复制到剪贴板","success")};window.reuseHistory=e=>{const o=t.history.find(s=>s.id===e);o&&(t.input=o.input,t.result=o.output,t.currentView="optimizer",d())};window.deleteHistoryItem=async e=>{confirm("确定删除这条历史记录？")&&(await U(e),n("✅ 已删除历史记录","success"),d())};window.clearHistory=async()=>{confirm("确定清空所有历史记录？此操作不可恢复！")&&(await B(),d())};window.exportHistoryJSON=()=>{const e=JSON.stringify(t.history,null,2),o=new Blob([e],{type:"application/json"}),s=URL.createObjectURL(o),r=document.createElement("a");r.href=s,r.download=`prompt-history-${new Date().toISOString().split("T")[0]}.json`,r.click(),URL.revokeObjectURL(s)};window.importHistory=()=>{const e=document.createElement("input");e.type="file",e.accept=".json",e.onchange=async o=>{const s=o.target.files[0];if(s)try{const r=await s.text(),a=JSON.parse(r);if(!Array.isArray(a)){n("❌ JSON 格式错误：必须是数组格式","error");return}const i=new Set(t.history.map(b=>b.id)),l=a.filter(b=>!i.has(b.id));t.history=[...l,...t.history],localStorage.setItem("history",JSON.stringify(t.history)),n(`✅ 成功导入 ${l.length} 条历史记录`,"success"),d()}catch(r){n("❌ 导入失败："+r.message,"error")}},e.click()};window.translateToEnglish=async()=>{t.isOptimizing=!0,d();try{const e=encodeURIComponent(t.result),o=await fetch(`https://api.mymemory.translated.net/get?q=${e}&langpair=zh|en`);if(!o.ok)throw new Error("翻译服务暂时不可用");const s=await o.json();if(s.responseStatus===200)t.result=s.responseData.translatedText;else throw new Error("翻译失败")}catch(e){n(`❌ 翻译失败：${e.message}`,"error")}t.isOptimizing=!1,d()};function v(){localStorage.setItem("backgroundImage",t.backgroundImage),localStorage.setItem("backgroundSize",t.backgroundSize),localStorage.setItem("backgroundOverlay",String(t.backgroundOverlay))}window.applyBackgroundUrl=()=>{const e=document.getElementById("backgroundUrl"),o=(e==null?void 0:e.value.trim())||"";if(!o){n("❌ 请输入图片 URL","error");return}t.backgroundImage=o,v(),d()};window.uploadBackgroundImage=e=>{var r;const o=(r=e.target.files)==null?void 0:r[0];if(!o)return;if(!o.type.startsWith("image/")){n("❌ 请选择图片文件","error");return}const s=new FileReader;s.onload=()=>{t.backgroundImage=String(s.result||""),v(),d()},s.readAsDataURL(o)};window.updateBackgroundSize=e=>{t.backgroundSize=e,v(),d()};window.updateBackgroundOverlay=e=>{t.backgroundOverlay=parseFloat(e),v();const o=document.getElementById("overlayValue");o&&(o.textContent=t.backgroundOverlay.toFixed(2));const s=document.getElementById("app-bg-overlay");s&&(s.style.opacity=String(t.backgroundOverlay))};window.clearBackgroundImage=()=>{t.backgroundImage="",v(),d()};window.saveSettings=()=>{var r;const e=document.getElementById("apiUrl").value,o=document.getElementById("apiKey").value,s=((r=document.getElementById("selectedModel"))==null?void 0:r.value)||t.selectedModel;t.apiUrl=e,t.apiKey=o,t.selectedModel=s,localStorage.setItem("apiUrl",e),localStorage.setItem("apiKey",o),localStorage.setItem("modelType",t.modelType),localStorage.setItem("selectedModel",s),n("✅ 设置已保存","success")};window.updateModelType=e=>{t.modelType=e};window.testConnection=async()=>{const e=document.getElementById("apiUrl").value,o=document.getElementById("apiKey").value,s=document.getElementById("testResult");if(!e||!o){s.innerHTML='<div class="border-2 border-black bg-red-100 p-4 text-sm">❌ 请先填写 API URL 和 API Key</div>';return}s.innerHTML='<div class="border-2 border-black bg-gray-100 p-4 text-sm">⏳ 测试连接中...</div>';try{const a=e.startsWith("http://")?`https://api.allorigins.win/raw?url=${encodeURIComponent(e+"/models")}`:`${e}/models`,i=await fetch(a,{method:"GET",headers:{Authorization:`Bearer ${o}`,...t.modelType==="claude"?{"anthropic-version":"2023-06-01"}:{}}});if(i.ok){const l=await i.json(),b=l.data||l.models||[];t.availableModels=b.map(p=>p.id||p.name||p).filter(Boolean),t.availableModels.length>0&&!t.selectedModel&&(t.selectedModel=t.availableModels[0]),s.innerHTML=`<div class="border-2 border-black bg-green-100 p-4 text-sm">✅ 连接成功！检测到 ${t.availableModels.length} 个可用模型</div>`;const c=s.previousElementSibling;if(t.availableModels.length>0&&!document.getElementById("selectedModel")){const p=`
          <div>
            <label class="font-bold text-sm mb-2 block text-gray-700 flex items-center gap-2">
              <span>✨</span>
              选择模型
            ...
            <select id="selectedModel" class="w-full border-2 border-black p-3 font-bold bg-white hover:bg-gray-50 smooth-transition depth-1 rounded-sm">
              ${t.availableModels.map(x=>`<option value="${x}" ${t.selectedModel===x?"selected":""}>${x}</option>`).join("")}
            </select>
          </div>
        `;c.insertAdjacentHTML("beforebegin",p)}}else{const l=await i.text();s.innerHTML=`<div class="border-2 border-black bg-red-100 p-4 text-sm">❌ 连接失败 (${i.status})：${l.substring(0,150)}</div>`}}catch(r){s.innerHTML=`<div class="border-2 border-black bg-red-100 p-4 text-sm">❌ 连接错误：${r.message}</div>`}};let y=null;window.selectTemplate=e=>{const s=u().find(r=>r.id===e||r.id===parseInt(e));s&&(t.selectedTemplate=s,t.currentView="optimizer",d(),n(`✅ 已选择模板：${s.name}`,"success"))};window.viewTemplate=e=>{const s=u().find(r=>r.id===e||r.id===parseInt(e));s&&(y=s,document.getElementById("modalTemplateName").textContent=`${s.icon} ${s.name}`,document.getElementById("editTemplateName").value=s.name,document.getElementById("editTemplateCategory").value=s.category,document.getElementById("editTemplateContent").value=s.systemPrompt,s.isCustom?(document.getElementById("editTemplateName").readOnly=!1,document.getElementById("editTemplateContent").readOnly=!1,document.getElementById("editTemplateCategory").readOnly=!1):(document.getElementById("editTemplateName").readOnly=!0,document.getElementById("editTemplateContent").readOnly=!0,document.getElementById("editTemplateCategory").readOnly=!0),document.getElementById("viewTemplateModal").classList.remove("hidden"))};window.hideViewTemplateModal=()=>{document.getElementById("viewTemplateModal").classList.add("hidden"),y=null};window.saveTemplateEdit=()=>{if(!y||!y.isCustom){n("❌ 内置模板不可修改","error");return}const e=document.getElementById("editTemplateName").value.trim(),o=document.getElementById("editTemplateCategory").value.trim(),s=document.getElementById("editTemplateContent").value.trim();if(!e||!o||!s){n("❌ 请填写完整信息","error");return}const r=m.findIndex(a=>a.id===y.id);r>-1&&(m[r].name=e,m[r].category=o,m[r].systemPrompt=s,localStorage.setItem("customTemplates",JSON.stringify(m)),hideViewTemplateModal(),d(),n("✅ 模板已更新！","success"))};window.clearTemplate=()=>{t.selectedTemplate=null,d()};window.importTemplateFile=()=>{const e=document.createElement("input");e.type="file",e.accept=".md,.txt",e.onchange=async o=>{const s=o.target.files[0];if(s)try{const r=await s.text(),a=s.name.replace(/\.(md|txt)$/,""),i=prompt("请输入模板分类（例如：编程、写作、分析）：","自定义");if(!i)return;z(a,i,r),d(),n(`✅ 成功导入模板：${a}`,"success")}catch(r){n("❌ 导入失败："+r.message,"error")}},e.click()};window.deleteTemplate=e=>{confirm("确定删除这个模板？")&&(A(e),n("✅ 已删除模板","success"),d())};window.addCurrentToFavorites=()=>{if(!t.result||!t.input){n("❌ 没有可收藏的内容","error");return}const e=t.selectedTemplate?t.selectedTemplate.name:"无模板";N(t.input,t.result,t.optimizeMode,t.selectedModel,e),n("⭐ 已添加到收藏夹！","success")};window.favoriteStyle=e=>{const o=w.find(a=>a.name===e);if(!o)return;if(f("style",e)){n("⚠️ 该风格已在收藏夹中","info");return}const s={id:Date.now(),type:"style",name:o.name,nameEn:o.nameEn,desc:o.desc,keywords:o.keywords,docUrl:o.docUrl,image:o.image,timestamp:new Date().toISOString()};t.favorites.push(s),localStorage.setItem("favorites",JSON.stringify(t.favorites)),n("⭐ 风格已收藏！","success");const r=document.querySelector(".flex-1.overflow-y-auto");r&&(r.innerHTML=h())};window.favoriteTemplate=e=>{const s=u().find(i=>i.id===e||i.id===parseInt(e));if(!s)return;if(f("template",e)){n("⚠️ 该模板已在收藏夹中","info");return}const r={id:Date.now(),type:"template",templateId:s.id,name:s.name,category:s.category,icon:s.icon,systemPrompt:s.systemPrompt,timestamp:new Date().toISOString()};t.favorites.push(r),localStorage.setItem("favorites",JSON.stringify(t.favorites)),n("⭐ 模板已收藏！","success");const a=document.querySelector(".flex-1.overflow-y-auto");a&&(a.innerHTML=h())};window.useFavoriteTemplate=e=>{const o=t.favorites.find(s=>s.id===e);!o||o.type!=="template"||(t.selectedTemplate={id:o.templateId,name:o.name,category:o.category,icon:o.icon,systemPrompt:o.systemPrompt},t.currentView="optimizer",d(),n("✅ 已切换到优化器并选择该模板！","success"))};window.reuseFavorite=e=>{const o=t.favorites.find(s=>s.id===e);o&&(t.input=o.input,t.result=o.output,t.optimizeMode=o.mode,t.currentView="optimizer",d())};window.editFavoriteNote=e=>{const o=t.favorites.find(s=>s.id===e);if(o){const s=prompt("添加备注（可选）：",o.note||"");s!==null&&(j(e,s),d())}};window.deleteFavoriteItem=e=>{confirm("确定删除这条收藏？")&&(C(e),n("✅ 已删除收藏","success"),d())};window.clearFavorites=()=>{confirm("确定清空所有收藏？此操作不可恢复！")&&(L(),d())};window.exportFavoritesJSON=()=>{const e=JSON.stringify(t.favorites,null,2),o=new Blob([e],{type:"application/json"}),s=URL.createObjectURL(o),r=document.createElement("a");r.href=s,r.download=`prompt-favorites-${new Date().toISOString().split("T")[0]}.json`,r.click(),URL.revokeObjectURL(s)};function d(){if(document.getElementById("app").innerHTML=`
  ...
  `,t.backgroundImage){const e=document.getElementById("app-shell"),o=document.getElementById("app-bg-overlay");e&&(e.style.backgroundImage=`url("${t.backgroundImage}")`,e.style.backgroundSize=t.backgroundSize,e.style.backgroundPosition="center",e.style.backgroundRepeat=t.backgroundSize==="auto"?"repeat":"no-repeat"),o&&(o.style.opacity=String(t.backgroundOverlay))}H()}d();
